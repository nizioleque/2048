...
int AreaData[AREA_COUNT][AREA_COUNT] = { {2,0,1,0},{2,0,2,0},{0,2,1,0},{2,0,2,0} };
...
void KeyUp() {
    for (int j = 0; j < AREA_COUNT; j++) {
        ...
        int empty = 0;
        while (i < AREA_COUNT) {
            // przeglądam kolumnę od góry

            if (AreaData[i][j] == 0) {
                // komórka pusta - przesuwam kolumnę w górę
                for (int k = i; k < AREA_COUNT - 1; k++) {
                    AreaData[k][j] = AreaData[k + 1][j];
                }
                AreaData[AREA_COUNT - 1][j] = 0;

                empty++;
                if (empty >= 4) break;
            }

            else if (i < AREA_COUNT - 1 && AreaData[i][j] == AreaData[i + 1][j]) {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   int windowSize = AREA_COUNT * (AREA_MARGIN + AREA_SIZE) + AREA_MARGIN;

   RECT rc;
   rc.top = 0;
   rc.bottom = windowSize;
   rc.left = 0;
   rc.right = windowSize;
   ...
   HWND hWndMirror = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPED,
       CW_USEDEFAULT, 0, rc.right - rc.left, rc.bottom - rc.top, nullptr, nullptr, hInstance, nullptr);
